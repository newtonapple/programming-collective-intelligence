module Similarity
  module Recommendations
    module ClassMethods
      ...
      def top_subject_matches(preferences, subject, n=5, similarity_metric='pearson_similarity')
        sort_and_trim! subject_matches(preferences, subject, similarity_metric), n
      end

      def top_item_matches(preferences, subject, n=5, similarity_metric='pearson_similarity')
        sort_and_trim! item_matches(preferences, subject, similarity_metric), n
      end


      def subject_matches(preferences, subject, similarity_metric='pearson_similarity')
        results             = []
      ...
      def item_matches(preferences, subject, similarity_metric='pearson_similarity')
      ...
      private 
        def disjoint_items(x,y)
          x.keys - y.keys
        end

        def sort_and_trim!(array, n)
    ...
    extend ClassMethods  # make all ClassMethods Recommendations' class methods as well.
  end # Recommendations   
end # Similarity